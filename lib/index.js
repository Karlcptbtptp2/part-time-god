...
function apply(ctx) {
  ...
  const ORB = ['................', '......yyyy......', '....yyyyyyyy....', '...yyyyyyyyyy...', '..yyyywwwwyyyy..', '..yyywwwwwwyyy..', '.yyyywwwwwwyyyy.', '.yyyywwwwwwyyyy.', '.yyyywwwwwwyyyy.', '..yyywwwwwwyyy..', '..yyyywwwwyyyy..', '...yyyyyyyyyy...', '....yyyyyyyy....', '......yyyy......', '................', '................']
  const LAMP_PX = ['......yy........', '.....ssss.......', '......ss........', '......ss........', '......ss........', '......ss........', '......ss........', '......ss........', '.....ssss.......', '....ssssss......', '....ssssss......', '................', '................', '................', '................', '................']
  const FISH_PX = ['................', '......oo........', '.....oooo.......', '....oookoo......', '...oooooooo.....', '..oooooooooo....', '...oooooooo.....', '....oooooo......', '.....oo.oo......', '.....o...o......', '................', '................', '................', '................', '................', '................']
  const BUTTERFLY_PX = ['......y..y......', '.....yy..yy.....', '...yyppppppyy...', '..ypppkkkkpppy..', '..ypppkkkkpppy..', '..yppppkkppppy..', '...yyppppppyy...', '.....yy..yy.....', '......y..y......', '................', '................', '................', '................', '................', '................', '................']

  // 组合式本地兜底：数百种组合，长度随乱码长度变化
  const FB_A = ['一只', '一群', '一朵', '一条', '一座', '一扇', '一位', '一片', '一团', '一阵']
  const FB_B = ['发光的', '温和的', '会唱歌的', '爱打盹的', '慢悠悠的', '透明的', '毛茸茸的', '金灿灿的', '胆小的', '热心的', '神秘的']
  const FB_C = ['蝴蝶', '锦鲤', '小怪兽', '白鹭', '风铃', '南瓜灯', '蘑菇', '云朵', '萤火虫', '稻草人', '热气球', '小精灵', '花仙子', '小狐狸', '纸灯笼', '风车', '小钟楼', '喷泉', '秋千', '信箱']
  const FB_D = ['出现了', '搬来同住', '亮了起来', '学会了新技能', '开始帮助村民', '悄悄安了家', '飘过天空', '在湖边定居', '开起了小店', '唱起歌来', '跳起了舞', '种下了一片花']
  const FB_E = ['，大家都很开心', '，孩子们围了过去', '，老周夸它勤快', '，夜里特别好看', '，湖面映着它的光', '，风一吹就响']
  function localDecode(seed, garble) {
    const pools = [FB_A, FB_B, FB_C, FB_D, FB_E]
    const segCount = 2 + (String(garble).length % 4)
    const parts = []
    let s = seed >>> 0
    for (let i = 0; i < segCount; i++) {
      const pool = pools[(i + (s % pools.length)) % pools.length]
      parts.push(pool[s % pool.length])
      s = (Math.imul(s, 1103515245) + 12345) >>> 0
    }
    const sentence = parts.join('')
    const pxPool = [BUTTERFLY_PX, FISH_PX, LAMP_PX, ORB]
    const kind = ['npc', 'object', 'plant', 'building'][fnv1a(String(seed) + ':k') % 4]
    return { sentence: sentence.slice(0, 28), kind, place: '任意空地', pixels: pxPool[fnv1a(String(seed) + ':p') % pxPool.length] }
  }
  ...
  async function decodeWithLlm(llm, adm, garble) {
    ...
    if (!parsed || typeof parsed.sentence !== 'string' || !parsed.sentence.trim()) {
      const raw = t.replace(/[\r\n]+/g, ' ').trim().slice(0, 28)
      return { sentence: raw || '神秘的光球出现了', kind: 'object', place: '任意空地', pixels: ORB }
    }
    ...
    return {
      ...
      pixels: pixels && !pixelsEmpty(pixels) ? pixels : ORB,
    }
  }

  function makeFeature(world, seed, id, impl, now) {
    const pixels = impl.pixels && !pixelsEmpty(impl.pixels) ? impl.pixels : ORB
    ...
    if (mode === 'theme') {
      ...
      try {
        ...
        if (llm && adm) {
          ...
          decoded = decoded || THEMES[fnv1a(String(seed)) % THEMES.length]
        } else {
          decoded = THEMES[fnv1a(String(seed)) % THEMES.length]
        }
      } catch (err) {
        decoded = THEMES[fnv1a(String(seed)) % THEMES.length]
      }
      ...
      try {
        ...
        const adm = ctx.get('agentDefaultModel')
        if (llm && adm) {
          impl = await decodeWithLlm(llm, adm, garble)
        } else {
          impl = localDecode(seed, garble)
        }
      } catch (err) {
