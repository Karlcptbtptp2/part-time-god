  ...
  factory: function (require) {
    ...
    function apply(ctx) {
      ...
      function zoomAt(cx, cy, factor) {
        var z2 = Math.min(2, Math.max(0.45, store.zoom * factor));
        var k = z2 / store.zoom;
      ...
      function GameWorldPet() {
        ...
        var onMove = function (e) {
          ...
          setStore({ petPos: { x: store.petDrag.ox + dx, y: store.petDrag.oy + dy }, petMoved: store.petMoved || Math.abs(dx) > 4 || Math.abs(dy) > 4 });
        };
        ...
        if (store.eventBubble && store.eventBubble.until > Date.now()) {
          ...
          bubble = React.createElement('div', { className: 'gw-event-bubble' },
            b.pixels ? React.createElement('div', { className: 'gw-thumb gw-spr', style: { backgroundImage: 'url("' + dynUrl(b.pixels) + '")' } }) : null,
      ...
      function buildDecor(sessionId, base) {
        ...
        for (var y = 0; y < ROWS; y++) {
          for (var x = 0; x < COLS; x++) {
            ...
            if (h % 100 < 26) deco.push({ s: kinds[h % kinds.length], x: x, y: y });
          }
        }
        return deco;
      }
        ...
        return React.createElement('div', { className: 'gw-map', onPointerDown: onPanStart, onPointerMove: onPanMove, onPointerUp: onPanEnd, onPointerCancel: onPanEnd },
          React.createElement('div', { className: 'gw-world', style: { width: WORLD_W + 'px', height: WORLD_H + 'px', transform: 'translate(' + cam.x + 'px,' + cam.y + 'px) scale(' + zoom + ')' } },
            ...
            (base.vehicles || []).map(function (v, i) {
              ...
              return React.createElement('div', { key: 'veh' + i, className: 'gw-obj gw-spr gw-spr-' + v.s + ' gw-click', style: { left: (vp.x + 0.5) * TILE + 'px', top: (vp.y + 0.5) * TILE + 'px' + bob, width: '44px', height: '44px', transform: 'translate(-50%, -55%)' }, onClick: function (e) { e.stopPropagation(); pickObj({ type: 'vehicle', i: i }); } });
            }),
            ...
            base.trees.map(function (t, i) { return React.createElement('div', { key: 'tree' + i, className: 'gw-obj gw-spr gw-spr-' + t.s + ' gw-click', style: obj(t, 48, 60), onClick: function (e) { e.stopPropagation(); pickObj({ type: 'tree', i: i }); } }); }),
            base.buildings.map(function (b, i) { return React.createElement('div', { key: 'bld' + i, className: 'gw-obj gw-spr gw-spr-' + b.s + ' gw-click', style: obj(b, 58, 62), onClick: function (e) { e.stopPropagation(); pickObj({ type: 'building', i: i }); } }); }),
