window.__ModuleLoader__.load({
  ...
  factory: function (require) {
    ...
    function dynUrl(pixels) {
      ...
      var url = 'data:image/svg+xml,' + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16" shape-rendering="crispEdges">' + rects.join('') + '</svg>');
        ...
        if (store.selectedFeat != null) {
          ...
          if (sf) {
            ...
            featTooltip = React.createElement('div', { className: 'gw-npc-tip', style: { left: Math.min(VIEW_W - 170, Math.max(6, fvx)), top: Math.min(VIEW_H - 56, Math.max(6, fvy - 44)) } },
              React.createElement('div', { className: 'gw-thumb gw-spr' + (sf.pixels ? '' : (sf.sprite ? ' gw-spr-' + sf.sprite : '')), style: sf.pixels ? { backgroundImage: 'url("' + dynUrl(sf.pixels) + '")' } : undefined }),
              React.createElement('div', null,
        ...
        return React.createElement('div', { className: 'gw-map', onPointerDown: onPanStart, onPointerMove: onPanMove, onPointerUp: onPanEnd, onPointerCancel: onPanEnd },
          React.createElement('div', { className: 'gw-world', style: { width: WORLD_W + 'px', height: WORLD_H + 'px', transform: 'translate(' + cam.x + 'px,' + cam.y + 'px) scale(' + zoom + ')' } },
            ...
            features.map(function (f) {
              var spriteCls = f.pixels ? '' : (f.sprite ? ' gw-spr-' + f.sprite : '');
              if (f.kind === 'npc') {
                ...
                if (f.pixels) st.backgroundImage = 'url("' + dynUrl(f.pixels) + '")';
                return React.createElement('div', {
                  key: 'feat' + f.id,
                  className: 'gw-obj gw-spr' + spriteCls + ' gw-npc gw-feat-npc',
                  style: st,
                  title: f.text,
              ...
              if (f.pixels) st2.backgroundImage = 'url("' + dynUrl(f.pixels) + '")';
              return React.createElement('div', { key: 'feat' + f.id, className: 'gw-obj gw-spr' + spriteCls + ' gw-feat', style: st2, title: f.text });
            }),
              ...
              world.log.slice().reverse().map(function (entry, i) {
                ...
                return React.createElement('div', {
                  ...
                  React.createElement('div', { className: 'gw-entry-main' },
                    entry.pixels ? React.createElement('div', { className: 'gw-thumb gw-spr', style: { backgroundImage: 'url("' + dynUrl(entry.pixels) + '")' } }) : (entry.sprite ? React.createElement('div', { className: 'gw-thumb gw-spr gw-spr-' + entry.sprite }) : React.createElement('span', null, '●')),
